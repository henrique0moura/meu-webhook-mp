/**
 * ============================================================================
 * WEBHOOK EXTERNO DO MERCADO PAGO — Deploy em Vercel, Railway, Render, etc.
 * ============================================================================
 *
 * COMO USAR:
 * 1. Crie um projeto em https://vercel.com (ou Railway/Render)
 * 2. Copie estes 2 arquivos para o projeto:
 *    - api/webhook.js        (este arquivo — recebe notificações do MP)
 *    - api/create-preference.js (cria o link de pagamento com external_reference)
 *    - package.json
 * 3. Configure as variáveis de ambiente (Environment Variables):
 *    MERCADO_PAGO_ACCESS_TOKEN  → Access Token do MP (produção)
 *    BASE44_APP_ID              → ID do seu app (pegue na URL do editor Base44)
 *    BASE44_ADMIN_EMAIL         → Email do seu admin
 *    BASE44_ADMIN_PASSWORD      → Senha do admin
 * 4. Deploy
 * 5. No painel do Mercado Pago → Webhooks → configure a URL:
 *    https://SEU-DOMINIO.vercel.app/api/webhook
 * 6. Eventos a escutar: "payment"
 * ============================================================================
 */
...
module.exports = async function handler(req, res) {
  ...
  // ── Consultar API do MP para validar o pagamento (nunca confiar no webhook) ──
  let paymentInfo;
  try {
  ...
  // ── external_reference é obrigatório ──
  ...
  // ── Login no Base44 como admin para atualizar a assinatura ──
  ...
  // ── Só ativa se status for "approved" ──
  if (status !== "approved") {
